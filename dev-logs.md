...
* 现在重现非常简单, 只需要执行 `setup` 脚本即可.
  不过需要确保环境配好了. 如果有时间, 改进脚本以检查环境.

## 6月8日
* 第一个线程 idle 可以在开中断的情况下执行了.
  问题就是, `cur_proc == next_proc` 的时候不需要换栈. ucore 里面也是这么写, 但是之前一直没注意.
